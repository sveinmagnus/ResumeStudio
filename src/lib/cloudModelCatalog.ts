 ...
 * live list yet.
 *
 * NO HARDCODED MODEL IDS. A curated shortlist of someone else's product
 * line-up is wrong the moment they ship, and it fails silently and late: a
 * stale Gemini entry suggested `gemini-2.5-flash` after Google retired it, and
 * the user only found out at "Save and test". `server/llmModels.ts` asks each
 * provider what it currently offers instead. What belongs here is placeholder
 * text and the nudge toward that live list — information that can't go stale.
 *
