...
export function importFromCVPartner(raw: Record<string, unknown>): ResumeStore {
  ...
  for (const p of rawProjects) {
    ...
    const project: Project = {
      ...
      end: yearMonth(p.year_to, p.month_to),
  ...
  for (const w of rawWork) {
    ...
    work_experiences.push({
      ...
      end: yearMonth(w.year_to as string, w.month_to as string),
      role_ids: [],
  ...
  for (const e of rawEdu) {
    educations.push({
      ...
      end: yearMonth(e.year_to as string, e.month_to as string),
  ...
  for (const c of rawCourses) {
    courses.push({
      ...
      start: null,
      end: yearMonth(c.year as string, c.month as string),
      skill_ids: [],
  ...
  for (const p of rawPos) {
    positions.push({
      ...
      end: yearMonth(p.year_to as string),
