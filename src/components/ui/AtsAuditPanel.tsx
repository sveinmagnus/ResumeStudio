...
export function AtsAuditPanel({ view }: { view: ResumeView }) {
  ...
  const [model, setModel] = useState<AtsModelResult | null>(null)
  const [error, setError] = useState<string | null>(null)

  const viewTextOut = useMemo(() => buildViewText(data, view, locale), [data, view, locale])

  const run = () => {
    setError(null); setModel(null)
    setCoverage(runLiteralAudit(data, view, locale, posting))
  }

  const gaps = coverage?.terms.filter((t) => t.status !== 'present').map((t) => t.term) ?? []

  const onModelResult = useCallback((text: string) => {
    setError(null)
    try {
      setModel(validateAtsResponse(JSON.parse(extractJson(text)), gaps))
    } catch (e) {
  ...
  return (
    <div className="ats">
      ...
      <div className="ats-actions">
        <button className="ats-run" onClick={run} disabled={!ready}>
