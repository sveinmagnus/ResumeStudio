...
- **`routes/backup.ts`**: the backup dir comes from `RESUME_BACKUP_DIR` (operator env), never from the request body — the client can't choose a filesystem path. Don't add a body-supplied path. The whole-store file also carries the instance registry (`StoreBackupV1.registry`); `parseStoreRegistry` is lenient (drops malformed entries) and `db.mergeRegistry` unions by key + skips empty id/key rows, so a tampered/garbage registry can't corrupt the merge or delete anything — the file is the operator's own synced data, not remote-attacker input.
