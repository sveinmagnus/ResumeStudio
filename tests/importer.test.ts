...
/**
 * The resume header and the four subsidiary sections.
 *
 * The subsidiary sections were entirely uncovered: certifications, positions,
 * presentations and honors/awards. CVpartner's field names do not match ours —
 * a position's ORGANISATION comes from `description`, a presentation's TITLE
 * comes from `description` too — so a wrong mapping does not fail, it silently
 * files the data under the wrong heading.
 */
describe('importFromCVPartner — resume header', () => {
  const imp = (raw: Record<string, unknown>) => importFromCVPartner(raw as never)

  it('takes the name from either the English or the Norwegian key', () => {
    expect(imp({ name: 'Kari' }).resume!.full_name).toBe('Kari')
    expect(imp({ navn: 'Kari' }).resume!.full_name).toBe('Kari')
    expect(imp({ name: 'Kari', navn: 'Ignored' }).resume!.full_name).toBe('Kari')
  })

  it('reads the phone from the Norwegian key, and nulls an absent one', () => {
    expect(imp({ telefon: '+47 900' }).resume!.phone).toBe('+47 900')
    expect(imp({}).resume!.phone).toBeNull()
  })

  it('builds a date of birth from the three parts, padding month and day', () => {
    expect(imp({ born_year: 1980, born_month: 3, born_day: 7 }).resume!.date_of_birth)
      .toBe('1980-03-07')
  })

  it('defaults a missing month or day to 1 rather than producing NaN', () => {
    expect(imp({ born_year: 1980 }).resume!.date_of_birth).toBe('1980-01-01')
  })

  it('leaves the date of birth null when no year is given', () => {
    // A day with no year is not a date; inventing one would put a wrong birthday
    // on the CV.
    expect(imp({ born_month: 3, born_day: 7 }).resume!.date_of_birth).toBeNull()
  })

  it('carries the twitter handle and the image URL through', () => {
    const r = imp({ twitter: '@kari', image: 'https://x.test/p.png' }).resume!
    expect(r.twitter).toBe('@kari')
    expect(r.profile_image_url).toBe('https://x.test/p.png')
    const bare = imp({}).resume!
    expect(bare.twitter).toBeNull()
    expect(bare.profile_image_url).toBeNull()
  })
...
describe('importFromCVPartner — the subsidiary sections', () => {
  ...
  describe('certifications', () => {
    const one = (over: Record<string, unknown> = {}) => imp({
      certifications: [{
        name: { en: 'AWS SA' }, organiser: { en: 'Amazon' },
        long_description: { en: 'Professional.' },
        year: '2022', month: '3', order: 2, starred: true, disabled: false, ...over,
      }],
    }).certifications[0]

    it('maps name, organiser, description and the issue date', () => {
      expect(one()).toMatchObject({
        name: { en: 'AWS SA' }, organiser: { en: 'Amazon' },
        description: { en: 'Professional.' },
        issued: { year: 2022, month: 3 },
      })
    })

    it('reads the description from long_description, not description', () => {
      // CVpartner keeps the prose in long_description; taking the short one
      // would drop the body of every certification.
      expect(one({ long_description: {}, description: { en: 'Short' } }).description).toEqual({})
    })

    it('maps an expiry only when one is given', () => {
      expect(one({ year_expire: '2025', month_expire: '3' }).expires).toEqual({ year: 2025, month: 3 })
      expect(one().expires).toBeNull()
    })

    it('carries order, starred and disabled, defaulting each', () => {
      expect(one()).toMatchObject({ sort_order: 2, starred: true, disabled: false })
      expect(one({ order: undefined, starred: undefined, disabled: undefined }))
        .toMatchObject({ sort_order: 0, starred: false, disabled: false })
    })

    it('imports nothing when the array is absent', () => {
  ...
  describe('positions', () => {
    ...
    it('files CVpartner’s description as the ORGANISATION, and name as the role', () => {
      // The two are the other way round from what the field names suggest.
      expect(one()).toMatchObject({
        name: { en: 'Board member' }, organisation: { en: 'Cartavio AS' },
      })
    })

    it('leaves the position description empty — CVpartner has no field for it', () => {
      expect(one().description).toEqual({})
    })

    it('maps the range, treating an EMPTY year_to as ongoing', () => {
      expect(one()).toMatchObject({ start: { year: 2020, month: null }, end: { year: 2022, month: null } })
      expect(one({ year_to: '' }).end).toBeNull()
      expect(one({ year_to: undefined }).end).toBeNull()
    })
  ...
  describe('presentations', () => {
    const one = (over: Record<string, unknown> = {}) => imp({
      presentations: [{
        description: { en: 'Scaling Postgres' }, long_description: { en: 'A talk.' },
        year: '2024', month: '9', order: 3, ...over,
      }],
    }).presentations[0]

    it('files description as the TITLE and long_description as the body', () => {
      expect(one()).toMatchObject({
        title: { en: 'Scaling Postgres' }, description: { en: 'A talk.' },
      })
    })

    it('leaves the event empty and puts the date on the range END (shape v13)', () => {
      // Courses and presentations became ranges; CVpartner supplies one date, so
      // it lands on `end` with `start` blank — the same rule as the migration.
      expect(one()).toMatchObject({
        event: {}, start: null, end: { year: 2024, month: 9 },
      })
    })

    it('imports nothing when the array is absent', () => {
      expect(imp({}).presentations).toEqual([])
    })
  })
  ...
  describe('honors and awards', () => {
    const one = (over: Record<string, unknown> = {}) => imp({
      honors_awards: [{
        name: { en: 'Employee of the Year' }, issuer: { en: 'Cartavio' },
        for_work: { en: 'The migration' }, long_description: { en: 'For it.' },
        year: '2021', month: '12', order: 4, ...over,
      }],
    }).honor_awards[0]

    it('maps name, issuer, for_work, description and date', () => {
      expect(one()).toMatchObject({
        name: { en: 'Employee of the Year' }, issuer: { en: 'Cartavio' },
        for_work: { en: 'The migration' }, description: { en: 'For it.' },
        date: { year: 2021, month: 12 },
      })
    })

    it('carries order and disabled, defaulting both', () => {
      expect(one()).toMatchObject({ sort_order: 4, disabled: false })
      expect(one({ order: undefined, disabled: true })).toMatchObject({ sort_order: 0, disabled: true })
    })

    it('imports nothing when the array is absent', () => {
      expect(imp({}).honor_awards).toEqual([])
    })
  })

  describe('spoken languages', () => {
    it('maps name and level, and defaults order and disabled', () => {
      const l = imp({ languages: [{ name: { en: 'Norwegian' }, level: { en: 'Native' } }] })
        .spoken_languages[0]
      expect(l).toMatchObject({
        name: { en: 'Norwegian' }, level: { en: 'Native' }, sort_order: 0, disabled: false,
      })
    })
  })
})
